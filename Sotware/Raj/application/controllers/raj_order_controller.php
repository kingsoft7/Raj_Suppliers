<?php 
class Raj_order_controller extends MY_Controller
{
	public function __construct(){
				parent::__construct();
				$this->load->model('raj_item_model','item');
				$this->load->model('raj_subItem_model','subItem');
				$this->load->model('raj_order_model','order');
				$this->load->model('raj_suppliers_model','supplier');
					}
	public function load_order_step_one_page(){
			$arr['orders']=$this->order->get_allTempOrderData();
			$arr['items']=$this->item->get_all_data();
			$arr['suppliers']=$this->supplier->get_all_data();
			$arr['tax']=$this->order->find_tax_details(1);
			$this->load->view('admin/raj_order_step_one',compact('arr'));
	}
	public function load_edit_order_tax_page(){
        $tax=$this->order->find_tax_details(1);
		$this->load->view("admin/raj_edit_p_tax",compact('tax'));
	}
	public function update_tax_Data(){				
					$post=$this->input->post();
					unset($post['submit']);
					if($this->order->update_tax_Data($post,1)){
					...
					return redirect('raj_order_controller/load_edit_order_tax_page');
	...
	public function selectItemData(){
		...
		$this->load->view('admin/raj_order_step_two',compact('arr'));
	}
	...
	public function saveTempData(){				
					...
					if($this->order->saveTempData($post)){
									$this->session->set_flashdata('feedback','Item Add Successfully');
									$this->session->set_flashdata('feedback_class','success');
								}
						else{
					...
					return redirect('raj_order_controller/load_order_step_one_page');
        ...
        $res=$this->order->saveOrder($data);
				if($res){
					...
					$this->order->moveData($tempOrder,$max);
									$this->session->set_flashdata('feedback','Order Recived Successfull');
									$this->session->set_flashdata('feedback_class','success');
								}
						else{
									$this->session->set_flashdata('feedback','Fail to Order Recived,Please Try Again');
									$this->session->set_flashdata('feedback_class','danger');
							}
					return redirect('raj_order_controller/load_order_step_one_page');
	}
public function view_order_page(){
	$this->load->library('pagination');
					$config=[
								'base_url'	=>	base_url('index.php/raj_order_controller/view_order_page'),
								'per_page'	=>10,
			...
			$this->load->view('admin/raj_view_purchase_order',compact('orders'));
}
public function load_view_old_order($order_id){
		...
		$this->load->view('admin/raj_view_purchase_order_details',compact('arr'));
