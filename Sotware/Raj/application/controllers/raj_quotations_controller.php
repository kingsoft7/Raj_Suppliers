<?php 
class Raj_quotations_controller extends MY_Controller
{
	public function __construct(){
				parent::__construct();
				$this->load->model('raj_item_model','item');
				$this->load->model('raj_subItem_model','subItem');
				$this->load->model('raj_quotations_model','order');
...
//print_r($tax);
	...
	public function update_tax_Data(){				
					...
					return redirect('raj_quotations_controller/load_edit_order_tax_page');
	...
	public function load_order_step_one_page(){
			...
			$this->load->view('admin/raj_quotations_step_one',compact('arr'));
	}
	...
	public function selectItemData(){
		...
		$this->load->view('admin/raj_quotations_step_two',compact('arr'));
	}
	...
	public function saveTempData(){				
					...
					return redirect('raj_quotations_controller/load_order_step_one_page');
        ...
        $res=$this->order->saveOrder($data);
				if($res){
					...
					$this->order->moveData($tempOrder,$max);
									$this->session->set_flashdata('feedback','Quotations Makes Successfull');
									$this->session->set_flashdata('feedback_class','success');
								}
						else{
									$this->session->set_flashdata('feedback','Fail to Make Quotations,Please Try Again');
									$this->session->set_flashdata('feedback_class','danger');
							}
					return redirect('raj_quotations_controller/load_order_step_one_page');
	}
public function view_order_page(){
	$this->load->library('pagination');
					$config=[
								'base_url'	=>	base_url('index.php/raj_quotation_controller/view_order_page'),
								'per_page'	=>10,
			...
			$this->load->view('admin/raj_view_quotations_order',compact('orders'));
}
public function load_view_old_order($order_id){
		...
		$this->load->view('admin/raj_view_quotations_order_details',compact('arr'));
