...
class Raj_order_model extends CI_Model{

	public function get_allTempOrderData(){
					$query=$this->db
								->from('tempOrder','subItem','item')
								->join('subItem', 'subItem.subItem_id = tempOrder.subItem_id')
	...
	public function saveTempData($data){
		try {
			 	$this->db->trans_begin();
   		   		$res= $this->db->insert('tempOrder', $data); 
	...
	public function getMaxRecord(){
			$query = $this->db->query("SELECT max(order_id) as id FROM purchase");
		    $row = $query->row();
		    return $row->id;
	}
 ...
 public function moveData($data,$id){ 
  		foreach ($data as $da) {
			         ...
			         $this->db->insert('purchasedetails');
	...
	public function get_storeData($limit,$offset){
				$query=$this->db
								...
								->join('suppliers', 'suppliers.supId = purchase.supId')
						 		->limit($limit,$offset)
	...
	public function get_order_details($order_id){
			$query=$this->db
						->from('purchasedetails','subItem','item')
						->join('subItem','subItem.subItem_id=purchasedetails.subItem_id')	
						->join('item', 'item.item_id = subItem.item_id')
